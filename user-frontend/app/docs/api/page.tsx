...
export default function ApiDocsPage() {
  ...
  const getSnippet = (endpoint: typeof ENDPOINTS[0]) => {
    if (selectedLang === 'curl') {
      return `curl -X ${endpoint.method} "http://localhost:8080${endpoint.path}" \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json" ${endpoint.body ? `\\
  -d '${endpoint.body}'` : ''}`;
    }
    if (selectedLang === 'js') {
      return `const res = await fetch("http://localhost:8080${endpoint.path}", {
      ...
      <div className={`pt-20 pb-32 ${isDark ? 'bg-gradient-to-b from-teal-500/10 to-transparent' : 'bg-gradient-to-b from-teal-50 to-transparent'}`}>
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
             <a href="http://localhost:8080/api/docs" target="_blank" className="px-8 py-4 bg-teal-600 text-white rounded-2xl font-bold hover:bg-teal-700 transition-all shadow-xl shadow-teal-600/20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-9 space-y-12">
            ...
            <div className={`p-10 rounded-[3rem] text-center border-2 border-dashed ${isDark ? 'bg-teal-500/5 border-teal-500/20' : 'bg-teal-50 border-teal-200'}`}>
               ...
               <a href="http://localhost:8080/api/docs" target="_blank" className="inline-flex items-center gap-2 bg-teal-600 text-white px-10 py-4 rounded-2xl font-bold text-sm hover:bg-teal-700 transition-all shadow-2xl shadow-teal-600/20">
