...
import { QrCode, ArrowRight, Mail, Lock, User, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSocialSignup = async (provider: 'google' | 'github') => {
    ...
    setError('');
    try {
      const authProvider = provider === 'google' ? 'mock-google' : provider;
      await signIn(authProvider, { callbackUrl: '/' });
    } catch {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'System error during account creation.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-[#F5F1EB] p-8 rounded-xl border border-[#D4CCC1] shadow-sm">
          ...
          <div className="relative mb-8">
            ...
            <div className="relative flex justify-center text-[10px] font-black uppercase tracking-[0.3em] text-[#9C958E]">
              <span className="bg-[#F5F1EB] px-4">Or sign up with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            <div>
              <label className="block text-xs font-bold text-[#6B6560] uppercase tracking-wider mb-2">Full Name</label>
              <div className="relative">
                <User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-teal-600" />
                <input
                  type="text"
                  value={name}
            ...
            <div>
              <label className="block text-xs font-bold text-[#6B6560] uppercase tracking-wider mb-2">Password</label>
              <div className="relative">
            ...
            <div className="flex items-center gap-2 py-1">
              <input type="checkbox" required className="w-4 h-4 text-teal-600 rounded border-[#B8AFA3] focus:ring-teal-500" />
          ...
          <div className="mt-8 text-center">
            <p className="text-[#6B6560] text-xs font-semibold">
              ...
              <Link href="/login" className="text-teal-600 font-bold hover:underline">Login</Link>
            </p>
          </div>
        </div>
      </div>
